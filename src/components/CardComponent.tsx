    ...
    <div className={`py-10 grid grid-cols-1 md:grid-cols-2 ${lendata === 2 ?"lg:grid-cols-2":"lg:grid-cols-3"} gap-6`}>
        {cardDetails?.map((detail: CardProps, index : number)=>(
            ...
            >
                ...
                <Button label={detail.CTA} variantStyle={index % 2 === 1 && lendata > 2 ?"whiteStyle": "noBgStyle"} rightIcon ={detail.CTA ? <FontAwesomeIcon icon={faArrowRightLong} className="hover:text-white" /> :""} />
            </div>
